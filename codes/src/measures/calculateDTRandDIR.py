...
#    eval_DIR = dIR(ranking, k, x)
    ...
    results.append([dataSetName, algoName, 'DTR_origin', eval_DTR_origin])
...
def dIR(ranking, k, x):
    ...
    proCTR = np.sum((np.sum((u[proListX]*v),axis=1)),axis=0)
    unproCTR = np.sum((np.sum((u[unproListX]*v),axis=1)),axis=0)
...
def solveLPWithoutFairness(ranking,algoName, k):
    ...
    #normalize input
    arrayU = (arrayU - np.min(arrayU))/(np.max(arrayU)-np.min(arrayU) + 0.0001)
